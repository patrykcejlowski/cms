...
function showMsgBox() {
	...
	div.id = 'ie6msgbox';
	div.innerHTML = ''
		+ '<img src="' + path + 'info.gif" width="16" height="16" class="infoicon" />'
	...
	function adjustSize() {
		body.runtimeStyle.paddingTop = parseInt(0+body.style.paddingTop) + div.offsetHeight + 'px';
		div.style.width = (document.compatMode != "CSS1Compat" ? document.body.clientWidth : document.documentElement.clientWidth - 29) + 'px';
	}
	body.appendChild(div);
	if (appVer < 7) {
		div.style.position = 'absolute';
		if (body.currentStyle.backgroundImage == 'none')
			body.style.backgroundImage = 'url(' + path + 'blank.gif)';
		body.style.backgroundAttachment = 'fixed';
		div.style.setExpression('top', "(document.compatMode != 'CSS1Compat' ? document.body : document.documentElement).scrollTop + 'px'");
		div.style.setExpression('left', "(document.compatMode != 'CSS1Compat' ? document.body : document.documentElement).scrollLeft + 'px'");
	}
	adjustSize();
	window.attachEvent('onresize', adjustSize);
	window.attachEvent('onscroll', adjustSize);
}
